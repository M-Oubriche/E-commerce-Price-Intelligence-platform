...
# ---- Replica of dbt int_clean_prices filters ----
_ACCESSORY_KEYWORDS = [
    "laptop stand", "notebook stand", "monitor stand", "laptop sleeve",
    "laptop cover", "phone case", "screen protector", "protective case",
    "housse", "chargeur", "sac à dos", "sac a dos", "cooling pad", "refroidisseur",
]

_MIN_PRICE = {
    "laptop": 100, "desktop": 100, "gpu": 50, "monitor": 50, "mobile": 50,
    "motherboard": 30, "cpu": 30, "case": 20, "psu": 15, "hdd": 10, "ssd": 10,
    "ram": 10, "keyboard": 5, "mouse": 5, "cooling": 5,
}

_MAX_PRICE = {
    "laptop": 10000, "desktop": 15000, "gpu": 6000, "monitor": 4000, "mobile": 3000,
    "motherboard": 1500, "cpu": 2500, "case": 1500, "psu": 1000, "hdd": 2000,
    "ssd": 2000, "ram": 1500, "keyboard": 800, "mouse": 500, "cooling": 1000,
}


def _passes_cleaning_filters(record):
    """Return True if this record would survive int_clean_prices.sql."""
    price = record.get("pricing", {}).get("converted_price_usd")
    if not price or price <= 0:
        return False
    name = (record.get("product", {}) or {}).get("name", "") or ""
    name_lower = name.lower()
    for kw in _ACCESSORY_KEYWORDS:
        if kw in name_lower:
            return False
    cat = ((record.get("product", {}) or {}).get("category", "") or "").lower()
    if cat in _MIN_PRICE and price < _MIN_PRICE[cat]:
        return False
    if cat in _MAX_PRICE and price > _MAX_PRICE[cat]:
        return False
    return True
...
def ingest_to_bigtable(json_str):
    ...
    for record in records:
        ...
        # ---- Look up previous price from Bigtable ----
        row_prefix = f"{category}#{brand}#{product_id}#{source}#".encode("utf-8")
        previous_price = None
        try:
            partial_rows = table.read_rows(
                filter_=row_filters.RowKeyRegexFilter(row_prefix + b".*")
            )
            last_row = None
            for r in partial_rows:
                last_row = r
            if last_row:
                cells = last_row.cells.get("price_cf", {}).get(
                    b"converted_price_usd", []
                )
                if cells:
        ...
        # ---- Alert detection (gated by dbt cleaning filters) ----
        if previous_price and current_price and _passes_cleaning_filters(record):
            if current_price < previous_price:
                ...
                if price_drop_percent >= 5.0:
                    is_drop = True
                    print(
                        f"!!! PRICE DROP DETECTED: {price_drop_percent:.2f}% !!!",
                        file=sys.stderr,
                    )
                    if pg_conn:
                        try:
                            cur = pg_conn.cursor()
                            prod_name = record.get("product", {}).get(
                                "name", "Unknown"
                            )
                            cur.execute(
                                "INSERT INTO alert_events "
                                "(product_id, product_name, source, old_price, new_price, drop_percent, is_processed) "
                            ...
                            print(
                                f"ERROR: Failed to insert alert: {e}",
                                file=sys.stderr,
                            )
                            pg_conn.rollback()
        ...
        # ---- Build Bigtable row ----
        row_key = (
            f"{category}#{brand}#{product_id}#{source}#{ts_str}".encode("utf-8")
        )
        ...
        row.set_cell(
            "ingestion_cf",
            b"raw_id",
            record.get("raw_id", str(uuid.uuid4())).encode("utf-8"),
        )
        row.set_cell(
            "ingestion_cf",
            b"ingestion_type",
            record.get("ingestion_type", "streaming").encode("utf-8"),
        )
        row.set_cell("ingestion_cf", b"scraped_at", scraped_at.encode("utf-8"))
        row.set_cell(
            "ingestion_cf", b"is_price_drop", str(is_drop).encode("utf-8")
        )
        row.set_cell(
            "ingestion_cf",
            b"price_drop_percent",
            f"{price_drop_percent:.2f}".encode("utf-8"),
        )
        if previous_price:
            row.set_cell(
                "ingestion_cf",
                b"prev_price_threshold",
                str(previous_price).encode("utf-8"),
            )

        prod = record.get("product", {})
        row.set_cell(
            "metadata_cf", b"name", prod.get("name", "").encode("utf-8")
        )
        row.set_cell(
            "metadata_cf", b"brand", prod.get("brand", "").encode("utf-8")
        )
        row.set_cell(
            "metadata_cf",
            b"category",
            prod.get("category", "").encode("utf-8"),
        )
        row.set_cell("metadata_cf", b"source", source.encode("utf-8"))
        row.set_cell(
            "metadata_cf",
            b"source_url",
            record.get("source_url", "").encode("utf-8"),
        )
        row.set_cell(
            "metadata_cf",
            b"external_id",
            prod.get("external_id", "Unknown").encode("utf-8"),
        )
        ...
        if prod.get("model_number"):
            row.set_cell(
                "metadata_cf",
                b"model_number",
                prod.get("model_number").encode("utf-8"),
            )
        if prod.get("description") is not None:
            row.set_cell(
                "metadata_cf",
                b"description",
                prod.get("description").encode("utf-8"),
            )
        if prod.get("image_url"):
            row.set_cell(
                "metadata_cf",
                b"image_url",
                prod.get("image_url").encode("utf-8"),
            )
        ...
        for p_field in [
            ...
            if pricing.get(p_field) is not None:
                row.set_cell(
                    "price_cf",
                    p_field.encode("utf-8"),
                    str(pricing.get(p_field)).encode("utf-8"),
                )
        ...
        if pricing.get("raw_currency"):
            row.set_cell(
                "price_cf",
                b"raw_currency",
                pricing.get("raw_currency").encode("utf-8"),
            )
        ...
        row.set_cell(
            "availability_cf",
            b"in_stock",
            str(avail.get("in_stock", False)).encode("utf-8"),
        )
        if avail.get("quantity") is not None:
            row.set_cell(
                "availability_cf",
                b"quantity",
                str(avail.get("quantity")).encode("utf-8"),
            )
        if avail.get("shipping_available") is not None:
            row.set_cell(
                "availability_cf",
                b"shipping_available",
                str(avail.get("shipping_available")).encode("utf-8"),
            )
        ...
        if seller.get("seller_name"):
            row.set_cell(
                "seller_cf",
                b"seller_name",
                seller.get("seller_name").encode("utf-8"),
            )
        if seller.get("seller_type"):
            row.set_cell(
                "seller_cf",
                b"seller_type",
                seller.get("seller_type").encode("utf-8"),
            )
        if seller.get("seller_rating") is not None:
            row.set_cell(
                "seller_cf",
                b"seller_rating",
                str(seller.get("seller_rating")).encode("utf-8"),
            )
        if seller.get("seller_location"):
            row.set_cell(
                "seller_cf",
                b"seller_location",
                seller.get("seller_location").encode("utf-8"),
            )
        ...
        if ratings and ratings.get("avg_rating") is not None:
            row.set_cell(
                "ratings_cf",
                b"avg_rating",
                str(ratings.get("avg_rating")).encode("utf-8"),
            )
        if ratings and ratings.get("review_count") is not None:
            row.set_cell(
                "ratings_cf",
                b"review_count",
                str(ratings.get("review_count")).encode("utf-8"),
            )
        ...
        if specs:
            ...
            if isinstance(specs, dict) and all(
                ...
                print(
                    "WARNING: Flat or malformed specs structure detected. Skipping specs ingestion.",
                    file=sys.stderr,
                )
